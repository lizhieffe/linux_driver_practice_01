...
MODULE_AUTHOR("lizhieffe");    ///< The author -- visible when you use modinfo
MODULE_DESCRIPTION("A simple Linux char driver for the BBB");  ///< The description -- see modinfo
MODULE_VERSION("0.1");            ///< A version number to inform users

static int majorNumber;  // device major number -- determined automatically
...
// Protoype functions for the character device -- must come before the struct definition
...
static int __init ebbchar_init(void){
  ...
  // Try to dynamically allocat a major number for the device.
  majorNumber = register_chrdev(0, DEVICE_NAME, &fops);
  if (majorNumber < 0) {
    printk(KERN_ALERT "EBBChar failed to register a major number\n");
    return majorNumber;
  }
  printk(KERN_INFO "EBBChar: device major number is %d\n", majorNumber);
